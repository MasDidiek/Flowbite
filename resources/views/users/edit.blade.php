    ...
    <div class="p-4 sm:ml-64 bg-gray-50">
        ...
        <div class="p-4 rounded-base mt-12 ">
            ...
            @php
                ...
                $bagian_list = ['Admin Loket', 'Farmasi', 'Laboratorium', 'Gigi', 'Gizi', 'Promkes','KIA/KB', 'Program', 'UKM', 'PJ Gudang Obat', 'PJ Alkes', 'PJ Barang', 'Surat', 'TU'];
            @endphp
                ...
                <form method="POST" action="{{ route('users.update', $user->id) }}">
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" required class="w-full border border-zinc-300 p-2 rounded-md" value="{{ $user->name  }}">
                        </div>

                        <div class="mb-3">
                            <label class="block mb-1">NIP</label>
                            <input type="text" name="nip" class="w-full border border-zinc-300 p-2 rounded-md" value="{{$user->nip}}">
                        </div>

                        <div class="mb-3">
                            <label class="block mb-1">Jabatan</label>
                            <input type="text" name="jabatan" class="w-full border border-zinc-300 p-2 rounded-md" value="{{$user->jabatan}}">
                        </div>
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div class="mb-3">
                            ...
                            <select name="puskesmas" class="w-full  border border-zinc-300 p-2 rounded-md">
                                    @foreach($puskesmas_list as $item)
                                        <option value="{{ $item }}"
                                            {{ $user->lokasi_kerja == $item ? 'selected' : '' }}>
                                            {{ $item }}
                                        </option>
                                    @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="email" name="email" required class="w-full border border-zinc-300 p-2 rounded-md" value="{{$user->email}}">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="username" required class="w-full border border-zinc-300 p-2 rounded-md" value="{{$user->username}}">
                        </div>
                    ...
                    <div class="bg-gray-50  py-3 sm:flex sm:flex-row-reverse sm:px-6">
                        ...
                        <a href="{{route('users.index')}}" class="mt-3 mr-2 inline-flex w-full justify-center rounded-md bg-gray-100 px-3 py-2 text-sm font-semibold text-gray-900 shadow-xs inset-ring inset-ring-gray-300 hover:bg-gray-200 sm:mt-0 sm:w-auto">Cancel</a>
                    </div>
